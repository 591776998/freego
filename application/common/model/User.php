...
class User extends model\Base{
    ...
    public function footprintNum(){
        return $this->hasOne('UserFootprint','user_id','id')->field('count(distinct s_code) as s_num,count(distinct c_code)+count(distinct city) as c_num,count(distinct place_id) as place_num,user_id');
        // return $this->hasMany('UserFootprint','user_id','id')->field('location_name,count(distinct location_value) as num,user_id')->group('location_name');
    }
    ...
    public function honour(){
        return $this->hasOne('honor','id','honor_id')->bind(['honor'=>'title']);
    }
