...
class BranchBase extends Controller
{
    ...
    public function _initialize()
    {
        ...
        //初始化当前管理员或者商家信息
//        session(config('admin_branch_key'),model('branch')->where('id',1)->find());

        $u = session(config('admin_branch_key'));
        if($u){
            $this->branch = $u;
        }else{
            if(isset($this->param['debug'])){
                $this->branch = model('branch')->where('id',1)->find();
            }
        }
