    ...
    public function index($now_user, $art_type,$tag_id = 0, $user_id, $forum_id, $key_word = '',$place_id = 0, $place_type = 0, $order = 'id', $is_my_follow = 0, $p = 1, $ps = 10, $is_best = 0, $status = 101, $is_collect = 0, $is_joined = 0,$location_name='',$location_value = ''){
        ...
        if(100 == $art_type){
            ...
            if($location_name && $location_value){
                $art_id_sql = model('locationArtRelation')->field('art_id')->where(function ($query)use($location_name,$location_value){
            ...
            if($forum_id){
                $art_model->alias('art')->field('art.*')->join('forum_top top',"art.id = top.art_id and top.status=101 and top.del_time=0 and (top.exp_time=0 or top.exp_time > UNIX_TIMESTAMP()) and (top.location_name='$location_name') and (top.location_value='$location_value')",'LEFT')->field('top.order_weight as tor')->order('tor desc');
            }
        }
...
//        $art_model = model('Art')->with($with)->withCount(['comments','answer','companion']);
...
//                        'city'              => $art_data['city'],
    ...
    protected function artActivity($art_id){
        ...
        if(!isset($data['lead_head_img_url'])){
        ...
        if($data){
            ...
            if($old){
                ...
                foreach ($data as $k => $v){
                    ...
                    if(!isset($v['s_code'])||!isset($v['p_code'])||!isset($v['c_code'])||!isset($v['t_code'])||!isset($v['city'])){
            ...
            if($data){
                ...
                if($data){
                    ...
                    $data = array_map(function($vo)use($id){
                         return [
                            'art_id'    => $id,
                            't_code'    => $vo['t_code'],
                            'c_code'    => $vo['c_code'],
