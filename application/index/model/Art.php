...
class Art extends model\Base{
    ...
    public function artJoin(){
        return $this->hasMany('artJoin','main_id','id');
    }

    public function forum(){
        return $this->belongsToMany('Forum', 'art_forum_relation', 'forum_id', 'art_id')->where('pivot.del_time','0');
    }
    ...
    public function comments(){
        return $this->hasMany('Comment','main_id','id')->with(['user','reply_user','imgs'])->where('type',103);
    }
    ...
    public function content(){
        //1.根据id查找对应的内容片断
        return $this->hasMany('ArtPara','main_id','id')->with(['img','video'])->where('status',101)->order('order_weight asc');
    }
